...
plan cd4pe::migrate::config(
  Target $cd4pe_4_target,
  Target $cd4pe_5_target,
) {
  $root_creds = run_command(
    'kubectl get secret cd4pe-root -o jsonpath=\'{.data}\'', $cd4pe_4_target)[0].value['stdout'].parsejson
  $root_password = base64('decode', $root_creds['password'])
  $root_email = base64('decode', $root_creds['email'])
  ...
  run_plan('cd4pe::generate_config', {
      admin_password       => Sensitive($root_password),
      admin_username       => $root_email,
      secret_key           => Sensitive($db_secret_key),
      admin_db_password    => Sensitive($admin_db_password),
