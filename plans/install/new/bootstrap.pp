...
plan cd4pe::install::new::bootstrap(
) {
  ...
  if $inventory_targets.size == 0 {
    ...
    out::message('Found multiple targets')
    $inventory_aio_target = prompt::menu('Select target to install CD4PE on', $inventory_targets)
  }
